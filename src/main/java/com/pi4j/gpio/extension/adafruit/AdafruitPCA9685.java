...
public class AdafruitPCA9685
{
	...
	public AdafruitPCA9685() throws UnsupportedBusNumberException
	{
		this(PCA9685_ADDRESS); // 0x40 obtained through sudo i2cdetect -y 1
	}

	public AdafruitPCA9685(int address) throws UnsupportedBusNumberException
	{
		try
		{
			// Get I2C bus
	...
	{
		double pulseLength = 1000000; // 1s = 1,000,000 us per pulse. "us" is to
										// be read "micro (mu) sec".
	 ...
	 * 1.5ms pulse | 0 deg | Stop 2ms pulse | 90 deg |FullSpeed forward 1ms
	 * pulse | -90 deg |FullSpeed backward
	 * ------------+----------+------------------
	 */
	public static void main(String[] args) throws UnsupportedBusNumberException
	{
		...
		servoBoard.setPWM(CONTINUOUS_SERVO_CHANNEL, 0, 0); // Stop the
															// continuous one
		servoBoard.setPWM(STANDARD_SERVO_CHANNEL, 0, 0); // Stop the standard
															// one
