...
public class MapStacker implements RobotLocationDeltaListener, DataSourceMap
{
	...
	public MapStacker(RobotInterface robot, RobotPoseSourceTimeTraveling robotPoseSource)
	{
		world.setDefaultValue(0);
		this.robotPoseSource = robotPoseSource;
		robot.addMessageListener(this);
		panel = new MapDrawingWindow("MapStacker", 0, 500, 1000, true);
	...
	public void onMessage(LidarScan lidarScan)
	{
		...
		new Thread(() -> {
			...
			ProbabilityMap matchingMap = ParticleFilterImpl.buildMatchingMap(world);
			...
			double[] params = new double[] {
					xy.getX().convert(DistanceUnit.CM), xy.getY().convert(DistanceUnit.CM), heading };
			...
			GDFunction function = new GDFunction()
			{
				...
				public double getValue(double[] parameters)
				{
					...
					{
						score -= new Particle(parameters[0], parameters[1], parameters[2], 0, 0)
								.scoreObservation(matchingMap, observation);
					}
					return score;
				}
			};

			double[] result = new GradientDescent(function, params).descend(0.1, 1);
			...
			for (ScanObservation observation : lidarScan.getObservations())
			{
				if (observation.getDisctanceCm() > 30)
				{
					...
					Vector3D offset = new Vector3D(result[0], result[1], 0);

					Rotation rotation = new Rotation(RotationOrder.XYZ, 0, 0, Math.toRadians(result[2]));
