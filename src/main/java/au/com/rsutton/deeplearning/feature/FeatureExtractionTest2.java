...
public class FeatureExtractionTest2
{
	...
	void dl4jText(RecordReader trainer, RecordReader test)
	{
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(seed).iterations(1)
				...
				// .layer(0,
				// new
				// DenseLayer.Builder().nIn(numInputs).nOut(numHiddenNodes).weightInit(WeightInit.XAVIER)
			...
			public List<Writable> next()
			{
				...
				if (pos % 3 == 1)
				{
					...
					List<Writable> convertFromPointsToAngles = convertFromPointsToAngles(
							getObtuseCorner(new Rotation(RotationOrder.XYZ, 0, 0, rand.nextInt(180)), 3));
	...
	List<Writable> convertFromPointsToAngles(List<Vector3D> list)
	{
		...
		double[][] image = new double[70][70];

		for (int i = 1; i < list.size(); i++)
		{
			image[(int) list.get(i).getX() + 35][(int) list.get(i).getY() + 35] = 1;
		...
		for (int x = 0; x < 70; x++)
		{
			for (int y = 0; y < 70; y++)
			{
				results.add(getWritable(image[x][y]));
			}
		}
		return results;
