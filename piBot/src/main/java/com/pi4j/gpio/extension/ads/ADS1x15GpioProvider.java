...
/**
 * <p>
 * This GPIO provider implements the TI ADS1x15 analog to digital converter chip
 * as native Pi4J GPIO pins.
 * 
 ...
 * </p>
 * 
 * TODO: Add support for ALARM pin using a GPIO to notify for events. This would
 * be more efficient than constantly polling each ADB pin in the monitoring
 * thread.
 * 
 * 
 ...
 * 
 */
public abstract class ADS1x15GpioProvider extends GpioProviderBase implements
		GpioProvider
{
	...
	protected int conversionDelay = 10;
	...
	protected static final int ADS1x15_REG_CONFIG_OS_SINGLE = 0x8000; // Write:
																		// Set
																		// to
																		// start
																		// a
																		// single-conversion
	protected static final int ADS1x15_REG_CONFIG_OS_BUSY = 0x0000; // Read: Bit
																	// = 0 when
																	// conversion
																	// is in
																	// progress
	protected static final int ADS1x15_REG_CONFIG_OS_NOTBUSY = 0x8000; // Read:
																		// Bit =
																		// 1
																		// when
																		// device
	...
	protected static final int ADS1x15_REG_CONFIG_MUX_DIFF_0_1 = 0x0000; // Differential
																			// P
																			// =
																			// AIN0,
																			// N
																			...
																			// AIN1
																			// (default)
	protected static final int ADS1x15_REG_CONFIG_MUX_DIFF_0_3 = 0x1000; // Differential
																			// P
																			// =
																			// AIN0,
																			// N
																			// =
																			// AIN3
	protected static final int ADS1x15_REG_CONFIG_MUX_DIFF_1_3 = 0x2000; // Differential
																			// P
																			// =
																			// AIN1,
																			// N
																			// =
																			// AIN3
	protected static final int ADS1x15_REG_CONFIG_MUX_DIFF_2_3 = 0x3000; // Differential
																			// P
																			// =
																			// AIN2,
																			// N
																			// =
																			// AIN3
	protected static final int ADS1x15_REG_CONFIG_MUX_SINGLE_0 = 0x4000; // Single-ended
																			// AIN0
	protected static final int ADS1x15_REG_CONFIG_MUX_SINGLE_1 = 0x5000; // Single-ended
																			// AIN1
	protected static final int ADS1x15_REG_CONFIG_MUX_SINGLE_2 = 0x6000; // Single-ended
																			// AIN2
	protected static final int ADS1x15_REG_CONFIG_MUX_SINGLE_3 = 0x7000; // Single-ended
																			// AIN3

	protected static final int ADS1x15_REG_CONFIG_PGA_MASK = 0x0E00;
	protected static final int ADS1x15_REG_CONFIG_PGA_6_144V = 0x0000; // +/-6.144V
																		// range
	protected static final int ADS1x15_REG_CONFIG_PGA_4_096V = 0x0200; // +/-4.096V
																		// range
	protected static final int ADS1x15_REG_CONFIG_PGA_2_048V = 0x0400; // +/-2.048V
																		// range
																		// (default)
	protected static final int ADS1x15_REG_CONFIG_PGA_1_024V = 0x0600; // +/-1.024V
																		// range
	protected static final int ADS1x15_REG_CONFIG_PGA_0_512V = 0x0800; // +/-0.512V
																		// range
	protected static final int ADS1x15_REG_CONFIG_PGA_0_256V = 0x0A00; // +/-0.256V
																		// range

	protected static final int ADS1x15_REG_CONFIG_MODE_MASK = 0x0100;
	protected static final int ADS1x15_REG_CONFIG_MODE_CONTIN = 0x0000; // Continuous
																		// conversion
																		// mode
	protected static final int ADS1x15_REG_CONFIG_MODE_SINGLE = 0x0100; // Power-down
																		// single-shot
																		// mode
																		// (default)

	protected static final int ADS1x15_REG_CONFIG_DR_MASK = 0x00E0;
	protected static final int ADS1x15_REG_CONFIG_DR_128SPS = 0x0000; // 128
																		// samples
																		// per
																		// second
	protected static final int ADS1x15_REG_CONFIG_DR_250SPS = 0x0020; // 250
																		// samples
																		// per
																		// second
	protected static final int ADS1x15_REG_CONFIG_DR_490SPS = 0x0040; // 490
																		// samples
																		// per
																		// second
	protected static final int ADS1x15_REG_CONFIG_DR_920SPS = 0x0060; // 920
																		// samples
																		// per
																		// second
	protected static final int ADS1x15_REG_CONFIG_DR_1600SPS = 0x0080; // 1600
																		// samples
																		// per
																		// second
																		// (default)
	protected static final int ADS1x15_REG_CONFIG_DR_2400SPS = 0x00A0; // 2400
																		// samples
																		// per
																		// second
	protected static final int ADS1x15_REG_CONFIG_DR_3300SPS = 0x00C0; // 3300
																		// samples
																		// per
																		// second

	protected static final int ADS1x15_REG_CONFIG_CMODE_MASK = 0x0010;
	protected static final int ADS1x15_REG_CONFIG_CMODE_TRAD = 0x0000; // Traditional
																		// comparator
																		// with
																		// hysteresis
																		// (default)
	protected static final int ADS1x15_REG_CONFIG_CMODE_WINDOW = 0x0010; // Window
																			// comparator

	protected static final int ADS1x15_REG_CONFIG_CPOL_MASK = 0x0008;
	protected static final int ADS1x15_REG_CONFIG_CPOL_ACTVLOW = 0x0000; // ALERT/RDY
																			// pin
																			// is
																			// low
																			// when
																			// active
																			// (default)
	protected static final int ADS1x15_REG_CONFIG_CPOL_ACTVHI = 0x0008; // ALERT/RDY
																		// pin
																		// is
																		// high
																		// when
	...
	protected static final int ADS1x15_REG_CONFIG_CLAT_MASK = 0x0004; // Determines
																		// if
																		// ALERT/RDY
																		// pin
																		// latches
																		// once
																		// asserted
	protected static final int ADS1x15_REG_CONFIG_CLAT_NONLAT = 0x0000; // Non-latching
																		// comparator
																		// (default)
	protected static final int ADS1x15_REG_CONFIG_CLAT_LATCH = 0x0004; // Latching
																		// comparator

	protected static final int ADS1x15_REG_CONFIG_CQUE_MASK = 0x0003;
	protected static final int ADS1x15_REG_CONFIG_CQUE_1CONV = 0x0000; // Assert
																		// ALERT/RDY
																		// after
																		// one
																		// conversions
	protected static final int ADS1x15_REG_CONFIG_CQUE_2CONV = 0x0001; // Assert
																		// ALERT/RDY
																		// after
																		// two
																		// conversions
	protected static final int ADS1x15_REG_CONFIG_CQUE_4CONV = 0x0002; // Assert
																		// ALERT/RDY
																		// after
																		// four
																		// conversions
	protected static final int ADS1x15_REG_CONFIG_CQUE_NONE = 0x0003; // Disable
																		// the
																		// comparator
																		// and
																		// put
	...
	// the threshold used to determine if a significant value warrants an event
	// to be raised
	...
	// this value defines the sleep time between value reads by the event
	// monitoring thread
	...
	public ADS1x15GpioProvider(int busNumber, int address) throws IOException
	{
		...
		// The first byte sent by the master should be the ADS1113/4/5 address
		// followed
		...
		// - First byte: 0b10010000 (first 7-bit I2C address followed by a low
		// read/write bit)
		...
		// - First byte: 0b10010001 (first 7-bit I2C address followed by a high
		// read/write bit)
		// - Second byte: the ADS1113/4/5 response with the MSB of the
		// Conversion register
		// - Third byte: the ADS1113/4/5 response with the LSB of the Conversion
		// register

		// set all default pin cache states to match documented chip power up
		// states
		// for (Pin pin : allPins) {
	...
	public ProgrammableGainAmplifierValue getProgrammableGainAmplifier(
			GpioPin pin)
	{
		return getProgrammableGainAmplifier(pin.getPin());
	}

	public void setProgrammableGainAmplifier(
			ProgrammableGainAmplifierValue pga, Pin... pin)
	{
		for (Pin p : pin)
		{
	...
	public void setProgrammableGainAmplifier(
			ProgrammableGainAmplifierValue pga, GpioPin... pin)
	{
		for (GpioPin p : pin)
		{
	...
	public double getImmediateValue(Pin pin) throws IOException
	{
		...
		int config = ADS1x15_REG_CONFIG_CQUE_NONE | // Disable the comparator
				...
				ADS1x15_REG_CONFIG_CMODE_TRAD | // Traditional comparator
												// (default val)
				ADS1x15_REG_CONFIG_DR_1600SPS | // 1600 samples per second
												// (default)
				ADS1x15_REG_CONFIG_MODE_SINGLE; // Single-shot mode (default)

		// Set PGA/voltage range
		config |= ADS1x15_REG_CONFIG_PGA_4_096V; // +/- 6.144V range (limited to
													// VDD +0.3V max!)
		...
		synchronized (sync )
		{
			...
			// read the conversion results
			 value = readRegister(ADS1x15_REG_POINTER_CONVERT);
		}
	...
	//	device.write((byte) register);
		...
		try
		{
			byteCount = device.read(register,buffer, 0, 2);
		} catch (IOException e)
		{
			e.printStackTrace();
		}
					...
					if (allPins != null && allPins.length > 0)
					{
						for (Pin pin : allPins)
						{
							...
							try
							{
								...
								// check to see if the pin value exceeds the
								// event threshold
								if (Math.abs(oldValue - newValue) > threshold[pin
										.getAddress()])
								{

									// cache new value (both in local event
									// comparison cache variable and pin state
									// cache)
									cachedValue[pin.getAddress()] = newValue;
									getPinCache(pin).setAnalogValue(newValue);

									// only dispatch events for analog input
									// pins
									if (getMode(pin) == PinMode.ANALOG_INPUT)
									{
										dispatchPinChangeEvent(
												pin.getAddress(), newValue);
									}
								}
		...
		private void dispatchPinChangeEvent(int pinAddress, double value)
		{
			...
			for (Pin pin : listeners.keySet())
			{
				...
				if (pin.getAddress() == pinAddress)
				{
					...
					{
						listener.handlePinEvent(new PinAnalogValueChangeEvent(
								this, pin, value));
					}
				}
			}
